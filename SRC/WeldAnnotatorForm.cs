...
namespace WeldAnnotator
{
    ...
    public class WeldPointAnnotatorCmd : TxButtonCommand
    {
        ...
        public override string LargeBitmap { get { return "WeldAnnotator.png"; } }
    ...
    public class WeldAnnotatorForm : TxForm
    {
        ...
        {
            Text = "焊点标注截图导出";
            // 新布局：左侧 270px 控制栏 + 右侧表格，1000×620 足够
            Size = new D.Size(1000, 620);
            MinimumSize = new D.Size(820, 480);
            // 打开位置：屏幕右侧，避免默认居中盖住 PS 视图中心
            StartPosition = FormStartPosition.Manual;
            try
            {
                var scr = Screen.PrimaryScreen.WorkingArea;
                Location = new D.Point(Math.Max(0, scr.Right - Size.Width - 20),
                                       Math.Max(0, scr.Top + 60));
            }
            catch { }
            Font = new D.Font("Tahoma", 9f);
            // 允许调整大小；用户可随时缩小窗口/拖到一侧，完全释放 PS 主视图交互
            FormBorderStyle = FormBorderStyle.Sizable;
            MaximizeBox = true;
            MinimizeBox = true;
            ShowInTaskbar = true;
            // 不 TopMost，让用户点击 PS 视图时窗口可以退到后台
            TopMost = false;

            // Dock 顺序：Fill 必须最后 Add，Dock=Left/Bottom 等先 Add。
            // 我们的需求：Bottom (log + status) → Left (rail) → Fill (table)
            BuildMainArea();    // Dock=Fill — Add 顺序最先，但因为是 Fill，
                                //             实际占据所有边缘 Dock 后剩余的空间
            BuildLogPanel();    // Dock=Bottom
            BuildStatusBar();   // Dock=Bottom
            BuildCardPanel();   // Dock=Left — 左侧控制栏（统一容纳所有非表格控件）

            // 关闭窗口前若存在未恢复的隐藏状态则弹窗确认
        ...
        {
            base.OnInitTxForm();
            // 不做 _opGrid.Focus()：打开窗口后让 PS 主视图保持输入焦点，
            // 用户可以直接在 PS 中选择/隐藏/调整视角，完全不受本窗口干扰。
        }
        ...
        private void BuildCardPanel()
        {
            ...
            WF.Label lblMode = new WF.Label
            {
                ...
                Font = new D.Font("Tahoma", 8f, D.FontStyle.Bold)
            };
            ...
            _btnExport.Font = new D.Font("Tahoma", 9.5f, D.FontStyle.Bold);
            ...
            _lblCount = new WF.Label
            {
                ...
                AutoSize = true,
                Dock = DockStyle.Top,
                Margin = new Padding(0, 6, 0, 6), // 底部也留点间距
        ...
        /// <summary>
        /// 创建左侧卡片：Dock=Top, 宽度固定 254，AutoSize 高度。
        /// 标题字体加粗，浅色边框，颜色协调。
        /// </summary>
        ...
        {
            WF.Button b = MkRailBtn(text, h, fg);
            b.Width = 232;
